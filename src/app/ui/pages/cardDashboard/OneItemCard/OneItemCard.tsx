...
export default function OneItemCard({amount, companyName, details, failureReason, avatar, amountUsd, method, type, status, createdAt }: transHistoryType) {
    ...
    <div className="w-full flex flex-col gap-0">
      <div className="grid md:grid-cols-12 grid-cols-11 w-full items-center gap-2 md:text-base text-xs">
        ...
        <div className="md:flex col-span-5 hidden justify-center items-center w-full">
          <div className={`w-fit px-2 py-0 rounded-full ${status == "rejected" ? "border border-solid border-[#FF8282] bg-[#FF8282] bg-opacity-15 text-[#FF8282]" : status == "pending" ? "border border-solid border-[#D79A00] bg-[#D79A00] bg-opacity-15 text-[#D79A00]" : ""} `}>
        ...
        <p className={`md:col-span-1 col-span-2 font-medium ${status == "pending" && type == "recharge" ? "text-[#D79A00]" : status == "failed" || status == "rejected" ? "line-through" : ""}`}>
          ...
          {amountUsd ? amountUsd
            ...
            (+(amount.toString()) + +(allDetails.relatedTransactions[0].amount.toString())).toFixed(2)
            :amount
          }
        </p>
        <p className="md:col-span-1 col-span-2 font-medium">
          {formattedDate}
        </p>
        <p className="md:col-span-1 col-span-2 font-medium">
          {formattedTime}
        </p>
        <ArrowBottomIcon className={`md:w-auto md:h-auto w-4 h-4 col-span-1 cursor-pointer fill-black dark:fill-white ${show ? "rotate-180" : ""} transition-all duration-500`} onClick={() => setShow(!show)} />
      </div>
      {
        allDetails &&
        <div className={`${show ? "block" : "hidden"}`}>
          <div className={`grid md:grid-cols-3 grid-cols-1 gap-2 md:text-base text-xs`}>
            <p>
              {t("bank-description")} {allDetails.bankDescription}
            </p>
            {failureReason && <p className="md:col-span-2">
          ...
          </div>
          {
            ...
            <p>
              Intl. Transaction Fee: {allDetails.relatedTransactions[0].amount.toString().slice(1)} $
            </p>
          }
        </div>
      }
