...
import { CircleDashedCheckIcon, CopyIcon, EyeClosedIcon, EyeIcon } from "@/src/app/icons/icons";
...
export default function ImageDetailsCard({ name, cvvNum, dateExp, id, cardId }: imageDetailsCardProps) {
  ...
  return (
    <div
      className="md:w-4/5 w-full col-span-1 h-fit text-white relative"
    >
      <Image
        src={srcImage}
        alt="card"
      ...
      <p className="absolute bottom-[33%] left-[10%] flex gap-2 md:text-lg sm:text-base text-sm">
        {
          showCardId ?
            groupedId?.map((group, index) => (
              <span key={index}>
                {group}
              </span>
            )) :
        ...
        {copy ? <CircleDashedCheckIcon className="cursor-pointer" /> : <CopyIcon className="cursor-pointer" onClick={() => handleCopy()} />}
