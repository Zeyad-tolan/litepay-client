...
export default function ActionsUserDetailsTwo({status}:{status:string}) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <button onClick={()=>deleteUser(user as string)} className="flex items-center text-lg font-medium px-4 w-fit border border-[#B83131] rounded-full bg-[#FFEAEA] text-[#B83131] gap-2">
        <IconDeleteUser /> {t('delete-user')}
      </button>
      {
        status === "active" ? 
        <button onClick={()=>blockUser(user as string)} className="flex items-center text-lg font-medium px-4 w-fit border border-[#B83131] rounded-full bg-[#FFEAEA] text-[#B83131] gap-2">
          <IconDeleteUser /> {t('ban-user')}
        </button>
        :
        <button onClick={()=>UnblockUser(user as string)} className="flex items-center text-lg font-medium px-4 w-fit border border-[#B83131] rounded-full bg-[#FFEAEA] text-[#B83131] gap-2">
