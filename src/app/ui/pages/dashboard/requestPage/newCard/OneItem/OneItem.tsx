...
export default function OneItem({ User, amount, amountUsd, method, nameOnCard, phoneNumber, attachments, status, account, createdAt, Card, cardId, id, telegram, type, updatedAt, userId }: itemRequest) {
  ...
  return (
    <tr className="border-t">
      ...
      <td className={`font-normal py-1 text-center`}>
        <span
          className={`flex justify-center mx-auto items-center gap-1 ${status === "pending"
      ...
      <td className={`font-normal py-1 text-center`}>
        ...
        {show && <PopUp User={User} setShowAdd={setShowAdd} amount={amount} amountUsd={amountUsd} method={method} nameOnCard={nameOnCard} phoneNumber={phoneNumber} attachments={attachments} status={status} account={account} createdAt={createdAt} setShow={setShow} Card={Card} cardId={cardId} id={id} telegram={telegram} type={type} updatedAt={updatedAt} userId={userId}/>}
        {showAdd && <PopAddCard userId={userId} balance={`${amountUsd}`} id={`${id}`} setShow={setShowAdd} show={showAdd}/> }
        {showImage && <PopUpImage src={attachments && attachments[0].length > 1 ? attachments[0] : `${srcImage}`} setShow={setShowImage} show={showImage}/>}
      </td>
    </tr>
  )
}
