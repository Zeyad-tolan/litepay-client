...
export default function MainCardDetails() {
  ...
  return (
    <>
    {
        dataCard && dataTrans && dataReqs && paginationReqs && paginationTrans &&
        <>
          <HeroCardDetails />
          <HeaderCardDetails cvv={dataCard.cvv} expiryDate={dataCard.expiryDate} status={dataCard.status} cardBalance={`${dataCard.balance}`} cardCreated={dataCard.createdAt} cardNumber={dataCard.cardNumber} email={dataCard.User.email} last30Days={`${dataCard.totalLast30DaysDeposit}`} phoneNumber={`${dataCard.User.phoneNumber}`} totalDeposits={`${dataCard.balance}`} user={dataCard.name} />
          {
            dataTrans && dataReqs &&
            <CardTransactions data={[...dataTrans,...dataReqs]} paginationReqs={paginationReqs} paginationTrans={paginationTrans}/>
          }
        </>
      }
    </>
