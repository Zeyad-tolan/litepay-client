...
export default function FormChargeCard() {
  ...
  const commission = user.Role ? user.Role.type === "user" ? normalUser : vipUser : normalUser
  ...
  useEffect(() => {
    ...
    if (focusInputName == "dollar") {
      ...
      sessionStorage.setItem("pound-value", `${Math.ceil(baseValue + fees + additionalCharge + cardPrice)}`)
    }

    if (focusInputName == "pound") {
      setDollarValue(
        typeCharge === "vodafone" ?
          ((poundValue - cardPrice) / (currencyDifference * (1 + commission + vodafoneCashFee))) :
          ((poundValue - cardPrice) / (currencyDifference * (1 + commission + instaFee)))
      );
  ...
  return (
    <form
      ...
      onSubmit={(e) => handelSubmitChargeCard(e, `${currencyDifference}`, promoCodeValue, typeCharge, lang?.toString() as string, router)}
    >
      <div className="w-full flex flex-col gap-2">
        <InputDollarChargeCart dollarValue={dollarValue} setDollarValue={setDollarValue} setFocusInputName={setFocusInputName} />
        <DetailsChargeCart cardPrice={priceCard} commission={commission} instapay={instapayValue} vodafoneCash={vodafoneCash} chargeType={typeCharge} currencyDifference={currencyDifference} />
