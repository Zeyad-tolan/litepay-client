...
export default function InputDollarChargeCart({
  dollarValue,
  setDollarValue,
  setFocusInputName,
}: InputDollarChargeCartProps) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <p className="font-light">{t("exactly")}</p>
      <div
        className={`flex gap-2 py-3 px-2 border border-solid ${
          dollarValue < 5 ? "border-amber-400" : "border-[#868685]"
        } rounded-xl`}
      >
        <input
          type="text"
          ...
          name="amountUsd"
          inputMode="numeric"
          className={`outline-none bg-transparent flex-1 min-w-14 ${
            dollarValue < 5 ? "text-amber-400" : ""
          }`}
      ...
      <p className="font-light">
        {!Number.isInteger(dollarValue) &&
          t("the-platform-only-accepts-whole-numbers").concat(
            " ",
            `${dollarValue.toFixed(2)}`,
            " ",
            t("will-be-rounded-to"),
            " ",
            `${Math.floor(dollarValue)}`
          )}
      </p>
      <p
