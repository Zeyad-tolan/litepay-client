...
export const handelSubmitPartOneForgotPassword = async (e: FormEvent<HTMLFormElement>, setPart: Dispatch<SetStateAction<1 | 2 | 3 | 4>>,setValueFirst: Dispatch<SetStateAction<string>> | undefined) => {
    e.preventDefault()
    ...
    try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}users/reset-password-req`, {
            method: "POST",
            headers: myHeaders,
            body: raw,
            redirect: "follow"
...
export const handelSubmitPartFourForgotPassword = (e: FormEvent<HTMLFormElement>,setErrorValue: Dispatch<SetStateAction<string>>) => {
    e.preventDefault()
    const formData = new FormData(e.currentTarget)
    const data = Object.fromEntries(formData.entries())
    // console.log(data);
    if (data.password != data.passwordConfirm) {
        ...
        .then(res=>{
            ...
            .catch(error => {
                ...
                toast.error(error.message || "An error occurred while reset the password.")
            })
    }
}